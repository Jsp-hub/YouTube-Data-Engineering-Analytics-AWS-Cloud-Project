"""
Glue Job: Bronze → Silver (Statistics Data - CSV data from Kaggle + JSON Data ingested from YT API)
────────────────────────────────────────────
...
Improvements over original pyspark_code.py:
  ...
  - Handles both Kaggle CSV format and API fetched JSON data
...
Our original kaggle CSV Files doesn't have 'region' column, while here we are using 'region' as our partition key to transform data
...
(USvideos.csv)
        ...
        ▼
Bronze Ingestion
(Adds region = "US")    - we are explicitly adding regions column - check in bronze glue data catalog
        │
        ▼
Bronze S3
+ Glue Catalog
(video_id, title, ..., region)
        │
        ▼
Your Silver Glue Job   - And out glue job is reading data directly from bronze catalog not from original CSV Data
        │
        ▼
Partition by region"""
...
SILVER_PATH = f"s3://{SILVER_BUCKET}/youtube/raw_statistics_data_l1transform/"
...
if initial_count == 0:
    ...
    # Standardize region codes to upper - so that new files are saved as region=US etc.
